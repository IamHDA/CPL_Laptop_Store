...
exports.getAll = async (req, res, next) => {
    try {
        ...
        const [products, total] = await Promise.all([
            Product.find(filter).select("-__v").populate("category", "name slug").sort(sort).skip(skip).limit(limit),
...
exports.getById = async (req, res, next) => {
    try {
        ...
        const product = await Product.findOne(query).select("-__v").populate("category", "name slug");
        ...
        const now = new Date();
        const flashSaleActive = product.isFlashSale && product.flashSaleEndsAt && product.flashSaleEndsAt > now;
    ...
    try {
        ...
        if (req.query.slugs) {
            const slugs = req.query.slugs
                ...
                .map((s) => s.trim())
                .filter(Boolean);
            categories = await Category.find({ slug: { $in: slugs }, isActive: true, deletedAt: null }).sort({
                sortOrder: 1,
                createdAt: 1,
            });
        } else {
            categories = await Category.find({ showOnHome: { $ne: false }, isActive: true, deletedAt: null }).sort({
                sortOrder: 1,
                createdAt: 1,
            });
        }
        ...
        const result = await Promise.all(
            categories.map(async (cat) => {
                const products = await Product.find({
                    ...
                    .select(
                        "name slug basePrice salePrice saleDiscount images isFlashSale flashSalePrice flashSaleEndsAt",
                    )
                    .sort({ sold: -1 })
                    .limit(8);
...
exports.createProduct = async (req, res, next) => {
    try {
        const { name, description, category, basePrice, salePrice, saleDiscount, stock, tags, images, specifications } =
            req.body;
...
exports.createVariant = async (req, res, next) => {
    try {
        ...
        const variantPrice =
            price !== undefined && price !== null ? Number(price) : (product.salePrice ?? product.basePrice);
...
exports.compareProducts = async (req, res, next) => {
    try {
        ...
        if (!ids) {
            return res
                .status(400)
                .json({ success: false, message: "Vui lòng truyền danh sách ID sản phẩm cần so sánh." });
        }
        const idList = ids
            .split(",")
            .map((id) => id.trim())
        ...
        if (idList.length > 5) {
            return res.status(400).json({ success: false, message: "Chỉ so sánh tối đa 5 sản phẩm." });
        }
        const products = await Product.find({
            _id: { $in: idList },
            isActive: true,
